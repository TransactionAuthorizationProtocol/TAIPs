...
<!--Please add an explicit list of intra-actor assumptions and known risk factors if applicable. Any normative definition of an interface requires these to be implementable; assumptions and risks should be at both individual interaction/use-case scale and systemically, should the interface specified gain ecosystem-namespace adoption. -->
Privacy legislations such as the General Data Protection Regulation (GDPR) and California Consumer Privacy Act (CCPA) enforce well established privacy principles such as "Data Minimisation" and "Purpose Limitation". In the context of analysing PII to comply with FATF's travel rule requirements, each agent must only process the minimum amount of data required for this purpose. TAIP-8 supports data protection legislations by giving each agent granular control over their PII requests, and provides travel rule creators with increased control to carefully consider whether the PII can be shared. <TBC>
