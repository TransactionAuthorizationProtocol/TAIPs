...
#### Scenario 1: Beneficiary Requests Originator [IVMS-101] Data
...
#### Scenario 2: Proactive Identity Sharing by the Originator
...
2. **Proactive Presentation:**  
   - Without waiting for an updatePolicies (RequirePresentation) message, the Originator's agent proactively creates a **Presentation** message that includes a Verifiable Presentation containing the [IVMS-101] data.
...
### Diagram 1: Identity Exchange via UpdatePolicies
...
sequenceDiagram
    ...
    B->>O: Transfer (transaction proposal, no PII)
...
### Diagram 2: Proactive [IVMS-101] Data Submission
...
sequenceDiagram
    ...
    O->>B: Transfer (transaction proposal)
    note right of B: Beneficiary confirms basic policies
    O->>B: Proactive Presentation (PII sent without explicit request)
    B->>O: Verify Presentation
    B->>O: Authorize
    O->>B: Settle
```
