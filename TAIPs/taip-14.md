...
**Partial Payments Handling:** In some cases a customer may not be able to pay the full amount at once or may accidentally underpay. Originally, an `allowPartial` flag in the request indicated whether partial payments were permitted. We remove this flag to simplify the protocol—**partial payments are implicitly possible** and whether to accept them is determined by the customer's wallet and the merchant's policy. This change reflects that ultimately a wallet (originator's agent) decides if it will allow sending a partial amount (and possibly follow up with additional payments) to fulfill the total. Removing `allowPartial` avoids confusion and duplication of policy logic between parties.
...
#### Figure 2: Detailed Payment Flow with Required Presentation (Successful Payment)
...
#### Figure 3: Payment Request Flow with Cancellation (Failure Scenario)
...
- **Partial Payment Risks:** Without an explicit `allowPartial` flag, there's a possibility of confusion if a partial payment is made unknowingly. Wallets should make it clear if a payment will not cover the full requested amount. Merchants should treat unexpected partial payments carefully – for example, either automatically refund or hold them pending completion. If partial payments are acceptable, malicious behavior (like a customer intentionally paying a tiny amount to lock an invoice) should be mitigated by merchant policy (e.g. cancel the request if full amount not received in time). These scenarios are left to business logic, but parties should be aware of them.

- **Cancel/Reject Handling:** Introducing Cancel messages adds a new state – wallets must handle a Cancel gracefully. A malicious or erroneous Cancel could conceivably be sent by an attacker, but since the channel is authenticated, that would imply the legitimate counterparty decided to cancel. Upon receiving a Cancel, the customer's wallet should not allow the user to send funds to that PaymentRequest's address through the app interface (to prevent paying a canceled invoice by accident). Similarly, the merchant's agent, after sending Cancel, should ignore any late incoming authorization or payment for that request. Both sides should ensure a canceled thread cannot be resurrected without a fresh PaymentRequest.
